...
def _snapshot_agent(agent: DyadAgent) -> Dict[str, Any]:
    ...
    return {
        ...
        "utterance_to_meaning": {u: _normalize(agent.belief_u2m.get(u, {})) for u in agent.utterances},
...
class Population:
    ...
    def __init__(
        ...
        self._last_round_dyad_beliefs: List[Dict[str, Any]] = []
    ...
    def _task_schedule(self, total_rounds: int, *, rng: Optional[random.Random] = None) -> List[str]:
        rng = rng or self.rng
    ...
    def learn_generation(
        ...
        """
        mode = training_mode
        n_rounds = rounds_per_dyad
        records: List[Dict[str, Any]] = []
        round_history: List[Dict[str, Any]] = []
        dyad_belief_history: List[Dict[str, Any]] = []
        ...
        def _record_dyad_beliefs(round_idx: int) -> None:
            dyad_belief_history.append(
                {
                    "round": round_idx,
                    "mode": mode,
                    "dyads": [
        ...
        # use a separate RNG for scheduling so dyad shuffles stay consistent across different total rounds
        schedule_rng = random.Random(self.rng.random())

        if mode == "paired":
            schedule = self._task_schedule(n_rounds, rng=schedule_rng)
            for round_idx, tower_id in enumerate(schedule):
                ...
                _record_mean(round_idx)
                _record_dyad_beliefs(round_idx)
        else:  # mixed
            ...
            schedule = self._task_schedule(total_rounds, rng=schedule_rng)
            ...
            for round_idx in range(n_rounds):
                block_start = round_idx * len(self.dyads)
                block_end = block_start + len(self.dyads)
                for seq_idx in range(block_start, block_end):
                    tower_id = schedule[seq_idx]
                    s_idx = speaker_sequence[seq_idx]
                    l_idx = listener_sequence[seq_idx]
        ...
        self._last_round_dyad_beliefs = dyad_belief_history
        return records
    ...
    def evaluate_generation(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        generation_result = {
            ...
            "dyad_belief_history": list(self._last_round_dyad_beliefs),
    ...
    def run_generation(
        ...
        n_rounds = rounds_per_dyad if rounds_per_dyad is not None else ROUNDS_PER_DYAD
        learned = self.learn_generation(
            rounds_per_dyad=n_rounds,
    ...
    def run(
        ...
        for _ in range(n_generations):
            self.run_generation(rounds_per_dyad=rounds_per_dyad, training_mode=training_mode)
        return self.history
...
def run_population(
    *,
    ...
    results_obj = {
        ...
        "dyad_belief_history": latest.get("dyad_belief_history", []),
    }
